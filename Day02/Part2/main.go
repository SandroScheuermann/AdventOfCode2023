...
const inputGames string = `Game 1: 18 red, 8 green, 7 blue; 15 red, 4 blue, 1 green; 2 green, 17 red, 6 blue; 5 green, 1 blue, 11 red; 18 red, 1 green, 14 blue; 8 blue
...
func main() {
	...
	for _, game := range games {
		...
		for _, set := range splittedGameSets {
			...
			splittedGameCubes := strings.Split(set, ",")

			for _, cubes := range splittedGameCubes {

				fmt.Sscanf(cubes, "%d %s", &setNumber, &setColor)

				if gameMap[setColor] < setNumber {
					gameMap[setColor] = setNumber
				}

			}
		}

		gamePower := (gameMap["red"] * gameMap["green"]) * gameMap["blue"]
